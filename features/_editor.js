...
// Mirrors the textwizard-tools scaffold so the wizard-family page shell and
// stylesheet work unchanged.
...
export function createEditor({
  ...
  function openSaveDialog() {
    ...
    let last = { name: 'punctuationwizard', ext: 'txt' };
    try {
    ...
    function save() {
      const name = (nameInput.value || 'punctuationwizard').trim() || 'punctuationwizard';
...
// Small inline prompt for tools that need user inputs. Returns a promise that
// resolves to a values object or null if cancelled. Uses the same modal
// styling as the Save dialog.
